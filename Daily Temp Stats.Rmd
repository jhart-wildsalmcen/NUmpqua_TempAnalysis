...
a <- ggplot(airwater_join, aes(x=air_max, y = water_max)) +
 ...
 labs(title="Delger Camp 8 Air & Water Correlation",
  ...
  geom_text(x = 23, y = 19, label = lm_eqn(airwater_join), parse = TRUE) +
  geom_text(x = 23, y = 18.5, label = "Pearson r = 0.86")
...
```{r}
daily_water_master <- daily_water_master %>% mutate(long = replace(long, logger_id == "21433141", "43.34425784"))
```
