...
```{r}
temp <- water_master %>% 
  filter(logger_id == "USGS Site 14316500 (Tokatee Falls)" & longdate > '2024-06-19' & longdate <= '2024-09-15') 
  

plot_21679182 <- ggplot() +
  geom_line(data=temp, aes(x=as.Date(longdate),y=mean_temp), linewidth =1,color='black') +
  geom_line(data=temp, aes(x=as.Date(longdate),y=min_temp), linewidth =0.3, color='blue') +
  geom_line(data=temp, aes(x=as.Date(longdate), y=max_temp), linewidth =0.3, color = 'red') +
  
  scale_x_date(date_breaks = "1 month", date_labels = "%b")+
  scale_y_continuous(breaks = seq(8,24,by=2)) +
  theme_bw()+
  labs(title= "Upper Copeland Cr.", subtitle = temp$coordinates, y="Temperature (°C)", x="Date")

plot_21679182

# Change file name 
ggsave(file="2024 Logger Sites/Water/Daily Plots/plot_21679182.png", height = 6, width = 10)
```


```{r}
daily_combined_plot <- plot_21433141 + plot_21433145 + plot_21679163 + plot_21679164 + plot_21679171 + plot_21679175 + plot_21679176 + plot_21679177 + plot_21679180 + plot_21679182
daily_combined_plot
